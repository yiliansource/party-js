...
```ts
emitter.options.modules.push((particle: Particle) => (particle.size = Math.sin(new Date().getTime() / 1000)));
```
...
```ts
type ModuleDriverValue<T> = T | Spline<T> | ((factor: number, particle?: Particle) => T);
```
