...
-   A rect object ( `{ x, y, width, height }` ).
-   An `HTMLElement`.
-   A `MouseEvent`.
...
<Accordion icon="/img/settings.svg" title="Configuration" expanded={true}>
    <FieldList>
        ...
        />
        <FieldItem
            name="opacityOverLifetime"
            type="ParticleModifier<number>"
            defaultValue="new NumericSpline({ time: 0, value: 1 }, { time: 0.5, value: 1 }, { time: 1, value: 0 })"
            description="The opacity that is applied to the sparkles over their lifetime."
        />
    </FieldList>
</Accordion>
